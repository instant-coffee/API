...
export class ProductsService {
  ...
  async getProduct(
    ...
    const shapedVariants: VariantDto[] = variants.map((v) => {
      ...
      for (const ptav of variantPtavs) {
        ...
        const attrName =
          lineId !== undefined ? lineIdToAttrName.get(lineId) : undefined;
    ...
    const shapedOptions: WheelOptionDto[] = noVariantLines.map((line) => {
      ...
      const values = line.product_template_value_ids
        ...
        .map((ptav) => ({
          ...
          label: ptav.name,
          ...(ptav.price_extra
            ? { priceExtra: this._formatPrice(ptav.price_extra, site.currency) }
            : {}),
        }));
  ...
  async getProductFamily(
    ...
    const templates = await this.odoo.searchRead<OdooProductTemplate>(
      "product.template",
      [
        ["product_tag_ids.name", "=", familyTag],
        ["active", "=", true],
      ],
      [
        "id",
    ...
    const allPtavIds = allAttrLines.flatMap(
      (l) => l.product_template_value_ids,
    );
    ...
    for (const { template, role } of roledTemplates) {
      ...
      const tmplVariants = allVariants.filter(
        (v) =>
          (v.product_tmpl_id as unknown as [number, string])[0] === template.id,
      );
    ...
    for (const { template, role } of roledTemplates) {
      const position = this._roleToPosition(role);
      const tmplLines = allAttrLines.filter((l) =>
        template.attribute_line_ids.includes(l.id),
      );
  ...
  async listProducts(site: SiteContext): Promise<
    (Pick<ProductResponseDto, "id" | "name" | "brand" | "currency"> & {
      familyTag?: string;
    })[]
  > {
    const templates = await this.odoo.searchRead<OdooProductTemplate>(
    ...
    const allTagIds = [
      ...new Set(templates.flatMap((t) => t.product_tag_ids ?? [])),
    ];
  ...
  private _attrNameToTypeKey(attrName: string): string {
    switch (attrName) {
      case FREEHUB_ATTRIBUTE:
        return "freehub";
      case BRAKE_ATTRIBUTE:
        return "brakeInterface";
      case FRONT_HUB_ATTRIBUTE:
        return "frontHub";
      case REAR_HUB_ATTRIBUTE:
        return "rearHub";
      case TORQUE_CAP_ATTRIBUTE:
        return "torqueCap";
      case VALVE_BRAND_ATTRIBUTE:
        return "valveBrand";
      case NOBL_CAP_ATTRIBUTE:
        return "noblCapColour";
      case PEATYS_COL_ATTRIBUTE:
        return "peatysColour";
      case GRAPHIC_ATTRIBUTE:
        return "graphicOption";
      case DECAL_COLOUR_ATTRIBUTE:
        return "decalColour";
      default:
        return attrName
          .toLowerCase()
