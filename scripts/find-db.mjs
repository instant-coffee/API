...
try {
  ...
  if (result.result?.length) {
    ...
    console.log(
      "   Blocked (expected on SaaS/SH):",
      result.error.data?.message ?? result.error.message
    );
  } else {
    console.log(
      "   Returned empty list — database listing is disabled on this instance."
    );
  }
} catch (e) {
...
console.log(
  "   2. DevTools → Network → log in → find POST /web/session/authenticate"
);
